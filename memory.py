...
class SequentialMemory(Memory):
    ...
    def sample_and_split(self, batch_size, batch_idxs=None):
        ...
        state0_batch = np.array(state0_batch).squeeze()
        #before it was .reshape(1,-1)
        state1_batch = np.array(state1_batch).squeeze()
        terminal1_batch = np.array(terminal1_batch).squeeze()
        reward_batch = np.array(reward_batch).squeeze()
        action_batch = np.array(action_batch).squeeze()
