...
DEFAULT_ACCEPT_START_DIST = 1

REWARD_INVALID_POSE = -1000

MAX_SPAWN_ATTEMPTS = 500000
...
class Simulator(gym.Env):
    ...
    def __init__(
        ...
        accept_start_dist = DEFAULT_ACCEPT_START_DIST,
        ...
        self.accept_start_dist = accept_start_dist
    ...
    def reset(self):
        ...
        for _ in range(MAX_SPAWN_ATTEMPTS):
            ...
            N = self.accept_start_dist
            ok = -M < lp.angle_deg < +M and (np.abs(lp.dist) <= 0.05)
            if not ok:
                continue
        ...
        self.cur_angle = 0
        self.cur_pos = np.array([0.5, 0, .42])
        #self.cur_pos = np.array([0.74, 0, 0.45])
    ...
    def _valid_pose(self, pos, angle, safety_factor=1.0):
        ...
        return (
                ...
                not self._collision(agent_corners) and
                point is not None
    ...
    def update_physics(self, action):
        ...
        self.delta_pos_x = self.cur_pos[0] - prev_pos[0]
        self.delta_pos_y = self.cur_pos[2] - prev_pos[2]
    ...
    def compute_reward(self, pos, angle, speed):
        ...
        i, j = self.get_grid_coords(pos)
        tile = self._get_tile(i, j)
        if tile['kind'] == 'curve_left' or tile['kind'] == 'curve_right':
            on_curve = True
        #print(self.delta_pos_x,self.delta_pos_y,angle* 180 / math.pi, lp.dot_dir)
        '''
        move_penalty = -1.5
        if on_curve:
            if np.abs(self.delta_pos_x) > 0.01 and np.abs(self.delta_pos_y) > 0.01:
                move_penalty = 0
            else:
                move_penalty = -1
        else:
            if lp.dot_dir >= 0.9 and lp.dot_dir <= 1.0:
                if np.abs(self.delta_pos_x) > 0.02 or np.abs(self.delta_pos_y) > 0.02:
                    move_penalty = 0
        '''
        '''
        #print(self.delta_pos_x)
        if not on_curve and (np.abs(self.delta_pos_x) > 0.005 or np.abs(self.delta_pos_y)) > 0.005:
           move_penalty = 0
        elif on_curve and (np.abs(self.delta_pos_x) > 0.001 or np.abs(self.delta_pos_y)) > 0.001:
            move_penalty = 0
        elif on_curve and (np.abs(self.delta_pos_x) < 0.001 or np.abs(self.delta_pos_y)) < 0.001:
            move_penalty = -1.0
        else:
        ...
        #if not on_curve:
        if (np.abs(self.delta_pos_x) >0.005 or np.abs(self.delta_pos_y)) > 0.005:
            move_penalty = 0
        else:
            move_penalty = -1.5

        #else:
            #if (np.abs(self.delta_pos_x) > 0.002 or np.abs(self.delta_pos_y)) > 0.002:
               # move_penalty = 0
            #else:
             #   move_penalty = -1.0
        ...
        '''
        '''
        reward = self.delta_pos*self.delta_time
        '''
        ##print(self.delta_pos)
        #if self.delta_pos < 0.01:
            #return -1
        #else:
            #return 1#self.delta_pos*self.delta_time
        #reward = .02 + 2*self.delta_pos*self.delta_time*np.cos(angle) + (-np.abs(lp.dist)/5)
